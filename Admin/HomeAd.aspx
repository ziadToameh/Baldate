<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/Admin.master" AutoEventWireup="true" CodeFile="HomeAd.aspx.cs" Inherits="Admin_HomeAd" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    <script type="text/javascript" charset="utf-8">
        $(document).ready(function () {

            $('#TableList').dataTable({
                "aaSorting": [[0, "desc"]]
            });

        });
					
    </script>
    <script type="text/javascript">
        $(function () {
            $('#homeAdNav>a').css('color', 'blue');
        });

    </script>
    <div id="divList">
        <div style="width: 100%; height: 70px;">
            <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                    <td width="80%">
                        <h2 style="text-decoration: underline;">
                            Events List</h2>
                    </td>
                    <td valign="middle" align="right">
                        <a href="HomeAdNew.aspx">
                            <img src="CSS/images/add.png" width="30px" height="30px" style="border: 0" />
                        </a>
                    </td>
                    <td>
                        <a href="HomeAdNew.aspx">Add</a>
                    </td>
                </tr>
                <tr>
                    <td colspan="3">
                        filter categories:
                        <asp:DropDownList ID="ddlEventCategory" runat="server">
                        </asp:DropDownList>
                        <asp:Button ID="btnfilter" runat="server" Text="Filter" OnClick="btnfilter_Click" />
                    </td>
                </tr>
            </table>
        </div>
        <br />
        <asp:Label ID="lblMessage" runat="server"></asp:Label>
        <br />
        <table id="TableList" width="100%" align="center" cellpadding="0" cellspacing="0"
            border="0" class="display">
            <thead>
                <tr>
                <th></th>
                    <th>
                        <asp:Label Width="100px" ID="Label1" Text="Name" runat="server"></asp:Label>
                    </th>
                    <th>
                        <asp:Label Width="100px" ID="Label3" Text="Date" runat="server"></asp:Label>
                    </th>
                    <th>
                    </th>
                    <th>
                    </th>
                    <th>
                    </th>
                    
                </tr>
            </thead>
            <tbody>
                <asp:Repeater ID="rptList" runat="server">
                    <ItemTemplate>
                        <tr>
                        <td>
                                <%# Eval("Id")%>
                            </td>
                           
                            <td>
                                <%# Eval("Name_Ar")%>
                            </td>
                             <td>
                                <%#DataBinder.Eval(Container.DataItem, "DateAdded", "{0:MMM d, yyyy}")%>
                            </td>
                            <td>
                                <a href="HomeAdEdit.aspx?id=<%# Eval("Id")%>">edit</a>
                            </td>
                            <td>
                                <a href="HomeAdDelete.aspx?id=<%# Eval("Id")%>&catId=<%# Eval("CategoryId")%>" onclick="return confirm('Are you sure you want to delete?')">
                                    delete</a>
                            </td>
                            <td>
                                <a href="HomeAdImage.aspx?eventid=<%# Eval("Id")%>&catId=<%# Eval("CategoryId")%>">Browse</a>
                            </td>
                        </tr>
                    </ItemTemplate>
                </asp:Repeater>
            </tbody>
        </table>
    </div>
</asp:Content>


